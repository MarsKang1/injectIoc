...
public class InjectViewId extends AnAction {
    private String mSelectedText;//选中的文字
    private FindViewByIdDialog mDialog;
    private int caretOffset;

    @Override
    public void actionPerformed(AnActionEvent e) {
        Project project = e.getProject();   // 获取project
        final Editor mEditor = e.getData(PlatformDataKeys.EDITOR); // 获取选中内容
        if (null == mEditor) return;
        SelectionModel model = mEditor.getSelectionModel();
        mSelectedText = model.getSelectedText();
        if (TextUtils.isEmpty(mSelectedText)) { // 未选中布局内容，显示dialog
            mSelectedText = getCurrentLayout(mEditor); // 获取光标所在位置的布局
